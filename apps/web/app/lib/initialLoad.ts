...
// the inbox/rooms requests fail (API hiccup, rate-limited endpoint, etc.) we
// still surface the user and only report the list failure — never leave the
// shell stuck on "Loading your conversations…".
...
export type InitialLoadCallbacks = {
  ...
  onLists: (dm: DMInboxEntry[], rooms: RoomInboxEntry[]) => void;
  onUnauthorized: () => void;
...
export async function loadInitialState(
  ...
  try {
    const [dm, rooms] = await Promise.all([api.getDmInbox(), api.getRooms()]);
    cb.onLists(dm.items, rooms.items);
  } catch {
